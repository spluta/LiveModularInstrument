FeedbackControl_Mod : Module_Mod {
	...
	*initClass {
		...
		StartUp.add {
			...
			SynthDef("feedbackControl_mod", {arg inBus, buf, buf2, volBus, thresh, mulFactor, attackReleaseFrames, sustainZeroFrames, waitGoFrames, tripCount, tripBlockFrames, topBin, id=0;
	...
	init {
		...
		this.makeMixerToSynthBus(1);
		{
			...
			synths.add(Synth.tail(group, "feedbackControl_mod" ,[\inBus, mixerToSynthBus, \buf, buf, \buf2, buf2, \volBus, volBus.index, \thresh, 0.8, \mulFactor, 0.7, \attackReleaseFrames, 200, \sustainZeroFrames, 50, \waitGoFrames, 100, \tripCount, 5,  \tripBlockFrames, 300, \topBin, 400, \id, id]));
