ReverbDrone_Mod : Module_Mod {
	...
	*initClass {
		StartUp.add {
			SynthDef("reverbDrone_mod", { arg out=0, bufnum, bufnum2, inBus, volBus, delayBus, distBus, gate = 1, pauseGate = 1;
				var in, in2, chain, chain2, outSig, trig, amp, local, env, pauseEnv, vol, dist, delTime;
				...
				in2 = PitchShift.ar(in, 0.2, 0.25, 0, 0.1);

				outSig = GVerb.ar(Mix.new(in), 100, 10, 0.5, 1, 0.5, 0, 0.1, 0.1);

				outSig = LPF.ar(outSig, 400);

				outSig = outSig+PitchShift.ar(outSig, 0.2, 0.5, 0, 0.1);
				outSig = outSig+ Resonz.ar(outSig, 100, 1);

				amp = Amplitude.kr(Mix.new(outSig));
				//outSig = outSig * (amp > 0.02)*4; // noise gate
	...
	init {
		...
		rout = Routine({
			group.server.sync;
			0.4.wait;
			synths.put(0, Synth("reverbDrone_mod", [\out, outBus, \bufnum, buffers[0].bufnum, \bufnum2, buffers[1].bufnum, \inBus, mixerToSynthBus.index, \volBus, volBus, \delayBus, delayBus, \distBus, distBus], group));
		});
		AppClock.play(rout);
	...
	killMeSpecial {
		buffers.do{|item| item.set(\gate, 0)};
	}
}